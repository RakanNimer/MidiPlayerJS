...
export class Track {
  ...
  handleEvent(currentTick, dryRun) {
    ...
    if (dryRun) {
      ...
      if (
        ...
        this.events[this.eventIndex].tick <= currentTick
      ) {
        this.eventIndex++;
  ...
  getStringData(eventStartIndex) {
    ...
    var byteCount = 1;
    var length = Utils.readVarInt(
      this.data.subarray(eventStartIndex + 2, eventStartIndex + 2 + byteCount)
    );
    var stringLength = length;

    return Utils.bytesToLetters(
      this.data.subarray(
        eventStartIndex + byteCount + 2,
        eventStartIndex + byteCount + length + 2
      )
    );
  }
    ...
    if (this.data[eventStartIndex] == 0xff) {
      ...
      switch (this.data[eventStartIndex + 1]) {
        ...
        case 0x21: // MIDI Port
          ...
          eventJson.data = Utils.bytesToNumber([
            this.data[eventStartIndex + 3]
          ]);
          break;
        ...
        case 0x51: // Set Tempo
          ...
          eventJson.data = Math.round(
            60000000 /
              Utils.bytesToNumber(
                this.data.subarray(eventStartIndex + 3, eventStartIndex + 6)
              )
          );
        ...
        case 0x58: // Time Signature
          ...
          eventJson.timeSignature =
            "" + eventJson.data[0] + "/" + Math.pow(eventJson.data[1], 2);
          break;
        case 0x59: // Key Signature
          ...
          if (eventJson.data[0] >= 0) {
            eventJson.keySignature =
              Constants.CIRCLE_OF_FIFTHS[eventJson.data[0]];
          } else if (eventJson.data[0] < 0) {
            eventJson.keySignature =
              Constants.CIRCLE_OF_FOURTHS[Math.abs(eventJson.data[0])];
          }
        ...
        default:
          eventJson.name =
            "Unknown: " + this.data[eventStartIndex + 1].toString(16);
          break;
      }
      ...
      if (this.data[eventStartIndex] < 0x80) {
        ...
        if (this.data[eventStartIndex] <= 0x8f) {
          ...
          eventJson.velocity = Math.round(
            this.data[eventStartIndex + 2] / 127 * 100
          );
