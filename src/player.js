...
export class Player {
  ...
  getTracks() {
    ...
    while (trackOffset < this.buffer.length) {
      if (
        Utils.bytesToLetters(
          this.buffer.subarray(trackOffset, trackOffset + 4)
        ) == "MTrk"
      ) {
        let trackLength = Utils.bytesToNumber(
  ...
  getSongTimeRemaining() {
    return Math.round(
      (this.totalTicks - this.tick) / this.division / this.tempo * 60
    );
  }
  ...
  /**
	 * Removes one listener for a given playerEvent
	 * @param {string} - Name of event to unsubscribe from.
	 * @return {Player}
	 */
  off(playerEvent) {
    if (this.eventListeners.hasOwnProperty(playerEvent)) {
      this.eventListeners[playerEvent].pop();
    }
    return this;
  }
